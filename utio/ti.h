...
public:
    ...
    typedef vector<capout_t>	keystrings_t;		///< List of key strings corresponding to EKeyDataValue enum.
    typedef gdt::coord_t	coord_t;
    typedef gdt::dim_t		dim_t;
public:
    const std::wstring terminfoDbPath;
    const std::string terminalName;
    //const gdt::dim_t width;
    //const gdt::dim_t height;
    ...
    void		Load (/*const wchar_t* termname = NULL*/);
    void		LoadEntry (stringstream& buf/*, const wchar_t* termname*/) const;
...
private:
    ...
    typedef vector<char>	acsmap_t;
    typedef vector<number_t>	progargs_t;
