...
#define TERMINFO_MAGIC        0432 // octal

#define BitMask(t,n)	(t(~t(0)) >> ((sizeof(t) * 8) - (n)))
...
static std::ostream& operator << 
  (std::ostream& os, const STerminfoHeader& th);

static std::istream& operator >>
  (std::istream& is, STerminfoHeader& th);

static void write_strz (std::ostream& os, const std::string& str);

template <class In, class Out>
static Out copy_n (In first, size_t m, Out res);

static std::ostream& operator << 
  (std::ostream& os, const STerminfoHeader& th)
{
  os.write ((const char*) &th, sizeof (th));
  return os;
}
...
static std::istream& operator >>
  (std::istream& is, STerminfoHeader& th)
{
  is.read ((char*) &th, sizeof (th));
  return is;
}

#if 0
static void align (std::istream& is, size_t alignment)
{
  const std::istream::pos_type curPos = is.tellg ();
  size_t over = curPos % alignment;
  if (over)
    is.ignore (alignment - over);
}

static void align (std::ostream& os, size_t alignment)
{
  const std::ostream::pos_type curPos = os.tellp ();
  size_t over = curPos % alignment;
  if (over)
    os.seekp (curPos + (std::streamoff) (alignment - over));
}

static std::string read_strz (std::istream& is)
{
  std::string res;
  res.reserve (10); // TODO const
  char c;
  while (!is && (is.get (c), c))
    res.push_back (c);
  return res;
...
template <class In, class Out>
static Out copy_n (In in, size_t m, Out res)
{
...
/// Default constructor.
CTerminfo::CTerminfo       
  (const std::wstring& _terminfoDbPath,
   const std::string _terminalName,
   gdt::dim_t _width,
   gdt::dim_t _height
   )
: m_AcsMap (acs_Last),
  ...
  m_nColumns (_width),
  m_nRows (_height),
  terminfoDbPath (_terminfoDbPath),
...
void CTerminfo::LoadEntry (stringstream& buf/*, const wchar_t* termname*/) const
{
    string tipath;
    std::wstring wname;
    wname.reserve (terminalName.length ());
    transform 
      (terminalName.begin (),
       terminalName.end (),
       back_inserter<std::wstring> (wname),
       to_wide_char
       );
    const std::wstring path = terminfoDbPath 
        + L'\\' + wname[0] 
        + L'\\' + wname;
    ifstream is (path.c_str (), std::ios::binary);
    if (!is)
      THROW_EXCEPTION 
        (SException, oss_ << L"File not found: " << path);
#if 1
    buf << is.rdbuf ();
#else
    int c = 0;
...
void CTerminfo::read (stringstream& is)
{
    // First the header.
    STerminfoHeader h;
    is >> h;
    if (h.m_Magic != TERMINFO_MAGIC)
        THROW_EXCEPTION (SException, L"Corrupt terminfo file");

    // The names section
    char* namesBuf = new char[h.m_NamesSize]; // FIXME check alloc
    is.read (namesBuf, h.m_NamesSize);
    if (namesBuf[h.m_NamesSize - 1] != 0)
      THROW_EXCEPTION
        (SException, L"Bad terminfo file");
    m_Name = namesBuf;
    delete [] namesBuf;

    // The boolean section
    m_Booleans.reserve (h.m_nBooleans);
    copy_n 
      (istreambuf_iterator<boolvec_t::value_type>(is), 
       h.m_nBooleans, 
       back_inserter (m_Booleans)
       );

    for 
      (boolvec_t::size_type i = 0; 
       i < h.m_nBooleans;
       i++
       )
     {
       boolvec_t::value_type b = m_Booleans.at (i);
       if (! (b == 0 || b == 1))
         THROW_EXCEPTION
          (SException, L"Bad terminfo file");
     }
    // Align
    if ((h.m_NamesSize + h.m_nBooleans) % 2)
      is.get (); // ignore one byte

    // The numbers section
    m_Numbers.resize (h.m_nNumbers);
    for 
      (numvec_t::size_type i = 0; 
       i < m_Numbers.size ();
       i++
       )
       is.read ((char*) &m_Numbers[i], sizeof (number_t));

    // The string offsets section
    m_Numbers.resize (h.m_nStrings);
    for 
      (stroffvec_t::size_type i = 0; 
       i < m_StringOffsets.size ();
       i++
       )
       is.read 
        ((char*) &m_StringOffsets[i], 
         sizeof (stroffset_t)
         );

    // The stringtable
    m_StringTable.reserve (h.m_StringTableSize);
    copy_n 
      (istreambuf_iterator<char> (is),
       h.m_StringTableSize,
       back_inserter (m_StringTable)
       );
}

/// Loads the terminfo entry \p termname.
void CTerminfo::Load (/*const wchar_t* termname*/)
{
    //assert (termname && termname[0]);
    stringstream tibuf;
    LoadEntry (tibuf/*, termname*/);
    read (tibuf);
    CacheFrequentValues();
    //ObtainTerminalParameters(); // FIXME
}
...
void CTerminfo::CacheFrequentValues (void)
{
    ...
    for (acsmap_t::size_type i = 0; i < acs_Last; ++ i)
...
#endif // TODO terminal window resizing ?
...
CTerminfo::capout_t CTerminfo::GetString (ti::EStrings i) const
{
    if (size_t(i) >= m_StringOffsets.size() || m_StringOffsets[i] == stroffset_t(ti::no_value))
        return "";
    return (m_StringTable.c_str () + m_StringOffsets[i]);
}
...
void CTerminfo::RunStringProgram (const char* program, string& result, progargs_t& args) const
{
       ...
       )
    {
        ...
        switch (*++i) {        // beware the excessive use of fallthrough :)
            ...
            case '-': PSPush (-(int) PSPop() + PSPop());        break;
            case '*': PSPush (PSPop() * PSPop());        break;
            ...
            case '~': PSPush (~PSPop());                break;
            case 't': bCondValue = PSPop() != 0;
            case 'e': if ((bCondValue = !bCondValue)) // this also supports elsif
                        --(i = min (std::search (i, prgstr.end (), 
                                                 c1.begin (), c1.end ()), 
                                    std::search (i, prgstr.end (),
                                                 c2.begin (), c2.end ())));
            case '?':
            case ';': break;
            case 'p': PSPush (args [min (std::string::size_type (*++i - '1'), args.size() - 1)]); break; // %p[0-9] pushes numbered parameter.
...
void CTerminfo::MoveTo (coord_t x, coord_t y, rstrbuf_t s) const
{
    ...
    args[0] = y;
    args[1] = x;
    RunStringProgram (GetString (ti::cursor_address), s, args);
    m_Ctx.m_Pos.x = x;
    m_Ctx.m_Pos.y = y;
}
...
{
  progargs_t args_fg(1), args_bg (1);
  args_fg[0] = fg;
  args_bg[0] = bg;
    if (m_Ctx.m_FgColor != fg && fg != color_Preserve)
        RunStringProgram (GetString (ti::set_a_foreground), s, args_fg);
    if (m_Ctx.m_BgColor != bg && bg != color_Preserve)
        RunStringProgram (GetString (ti::set_a_background), s, args_bg);
    m_Ctx.m_FgColor = fg;
    m_Ctx.m_BgColor = bg;
}
...
CTerminfo::capout_t CTerminfo::AttrOn (EAttribute a) const
{
    ...
    m_Ctx.m_Attrs |= (1 << a);
    return (a < attr_Last ? GetString (as[a]) : "");
}
...
CTerminfo::capout_t CTerminfo::AttrOff (EAttribute a) const
{
    ...
    if (a >= attr_Last || (as[a] == ti::exit_attribute_mode && GetString(ti::set_attributes) != ""))
        return (Attrs (newAttrs).c_str ());
...
void CTerminfo::Attrs (uint16_t a, rstrbuf_t s) const
{
    ...
    if (sgr == "") {
        ...
        uint16_t mask (1);
        for (size_t i = 0; i < attr_Last; ++ i, mask <<= 1) {
        ...
        progargs_t pa;
        for (progargs_t::size_type i = 0; i < pa.size(); ++ i)
            pa[i] = (a >> i) & 1;
...
CTerminfo::strout_t CTerminfo::Image (coord_t x, coord_t y, dim_t w, dim_t h, const CCharCell* data) const
{
    ...
    for (coord_t j = y; j < y + h; ++ j) {
        for (coord_t i = x; i < x + w; ++ i, ++ data) {
            ...
            if (!dc)
                continue;
            if (i != m_Ctx.m_Pos.x || j != m_Ctx.m_Pos.y) {
                if (i == 0 && j == m_Ctx.m_Pos.y + 1) {
                    m_Ctx.m_Output += '\n';
                    m_Ctx.m_Pos.x = 0;
                    ++ m_Ctx.m_Pos.y;
                } else
